//
//  NXMutableOrderedDictionary.m
//  NXFoundation
//
//  Created by caokeji on 2022/3/8.
//
...
@property (nonatomic, readonly) NXOrderedDictionary *root;

- (instancetype)initWithData:(NSData *)data  root:(NXOrderedDictionary *)root;

@end


@implementation NXOrderedDictionary
{
    @protected
    NSArray *_values;
    NSOrderedSet *_keys;
...
- (instancetype)init
{
    if ((self = [super init]) && [self class] == [NXOrderedDictionary class])
    {
        static NXOrderedDictionary *singleton;
...
{
    return [[NXMutableOrderedDictionary allocWithZone:zone] initWithDictionary:self];
}
...
@implementation NXMutableOrderedDictionary
...
{
    return [(NXMutableOrderedDictionary *)[self alloc] initWithCapacity:count];
}
...
{
    return [[NXOrderedDictionary allocWithZone:zone] initWithDictionary:self];
}
...
- (instancetype)initWithData:(NSData *)data root:(NXOrderedDictionary *)root
{
    if ((self = [super init]))
    {
        _root = [root isKindOfClass:[NXMutableOrderedDictionary class]] ? root : nil;
...
- (void)parser:(NSXMLParser *)parser didStartElement:(nonnull NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName attributes:(__unused NSDictionary<NSString *, NSString *> *)attributeDict
{
    if ([elementName isEqualToString:@"dict"])
    {
        if (_valueStack == nil)
        {
            _valueStack = [NSMutableArray arrayWithObject:_root ?: [NXMutableOrderedDictionary dictionary]];
        }
        else
        {
            [_valueStack addObject:[NXMutableOrderedDictionary dictionary]];
        }
    }
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName
{
    ...
    if ([top isKindOfClass:[NXMutableOrderedDictionary class]])
    {
        NSString *key = _keyStack.lastObject;
        ((NXMutableOrderedDictionary *)top)[key] = value;
